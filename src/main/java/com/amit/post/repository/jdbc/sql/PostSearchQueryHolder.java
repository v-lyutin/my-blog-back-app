...
public final class PostSearchQueryHolder {
    ...
    public static final String SEARCH_POSTS_BY_TITLE_QUERY_AND_TAGS = """
        SELECT p.id, p.title, p.text, p.likes_count, p.comments_count
        FROM posts p
        LEFT JOIN post_tag pt ON pt.post_id = p.id
        LEFT JOIN tags t      ON t.id = pt.tag_id
        WHERE (:query IS NULL OR p.title ILIKE :query) AND (:tagCount = 0 OR t.name IN (:tagNames))
        GROUP BY p.id
    ...
    public static final String COUNT_POSTS_BY_TITLE_QUERY_AND_TAGS = """
